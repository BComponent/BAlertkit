...
@property (nonatomic, strong) BPopupConfigure *config;//弹窗的配置
...
- (void)setConfig:(BPopupConfigure *)config {
    _config = config;
...
- (void)timerLoopExecute {
    if (self.dismissTime < 1) {
        [self closeTimer];
        //关闭弹窗
        [BPopupsManager dismissWithPopup:self.popupObj];
        return;
    }
    self.dismissTime --;
...
@property(nonatomic, strong, readonly) NSArray<NSString *> *allGroup;
@end

@implementation BPopupsManager

+ (instancetype)defaultManager {
    static dispatch_once_t onceToken;
    static BPopupsManager *instance = nil;
...
+ (void)addPopup:(id<BPopupInterface>)popup {
    BPopupsManager *manage = [BPopupsManager defaultManager];
...
+ (void)addPopupController:(UIViewController *)popController options:(BPopupConfigure * _Nullable )options{
    BPopupsManager *manage = [BPopupsManager defaultManager];
    BView * popView = [BView new];
    options.popViewDidShowCallback = ^{
        [BPopupsManager dismissWithPopup:popView];
        [BPopupsManager pushVC:popController];
    };
    [manage addPopup:popView
                   priority:0
                    options:options];
}

+ (void)addPopup:(id<BPopupInterface>)popup options:(BPopupConfigure *)options{
    BPopupsManager *manage = [BPopupsManager defaultManager];
    [manage addPopup:popup
                   priority:0
                    options:options];
...
//新增弹窗
- (void)addPopup:(id<BPopupInterface>)popup priority:(HLLPopupStrategyPriority)priority options:(BPopupConfigure *)options{
    if (!popup || ![[self class] checkMainThread]) {
        return;
    }
    //弹窗配置对象
    BPopupConfigure *config = [BPopupConfigure new];
    if (options) {
        config = [options copy];
    }else{
        if (priority != 0) {
    ...
    if (![popup conformsToProtocol:@protocol(BPopupInterface)]) {
        return;
    }
    //将弹窗内容视图添加到弹窗背景视图中
...
+ (void)dismissWithPopup:(id<BPopupInterface>)popup {
    ...
    HLLPopupModel *model = [[BPopupsManager defaultManager] getModelWithPopup:popup];
    if (model) {
        [[BPopupsManager defaultManager] dismissWithModel:model];
    }
}
...
+ (void)dismissPopupWithIdentifier:(NSString *)identifier {
    ...
    HLLPopupModel *model = [[BPopupsManager defaultManager] getModelWithIdentifirer:identifier];
    if (model) {
        [[BPopupsManager defaultManager] dismissWithModel:model];
    }
}
...
+ (void)removeAllPopupFromContainerView:(UIView *)containerView {
    ...
    NSArray *arr = [[BPopupsManager defaultManager] getAllPopViewFromContainerView:view];
    if (arr.count < 1) {
        return;
    }
    ...
    [[BPopupsManager defaultManager].windowQueue removeObjectsInArray:arr];
}

+ (void)removeAllPopup {
    NSMutableArray *array = [[NSMutableArray alloc] init];
    for (HLLPopupModel *itemModel in [BPopupsManager defaultManager].windowQueue) {
        //剔除通知条类型的
    ...
    [[BPopupsManager defaultManager].windowQueue removeObjectsInArray:array];
...
+ (NSInteger)getTotalPopupCount {
    ...
    return [BPopupsManager defaultManager].windowQueue.count;
}

#pragma mark - Pop

- (void)addSuperViewWithModel:(HLLPopupModel *)model{
...
- (void)popWithModel:(HLLPopupModel *)model isRecover:(BOOL)isRecover {
    ...
    void(^clearAgoPopsBlock)(NSArray *) = ^(NSArray *list){
        ...
        //移除之前所有的弹窗
        while (waitRemoveArr.count) {
            [self dismissWithModel:waitRemoveArr.lastObject isRemoveQueue:YES animation:NO];
            [waitRemoveArr removeLastObject];
        }
    };
    ...
    if(model.config.isTerminatorMode){//isTerminatorMode模式把不同的Group下的弹窗都给移除
        ...
        if (allPopModelArr.count && !isRecover) {
            ...
            if (model.config.priority >= lastModel.config.priority) {
                ...
                //当前组展示的弹窗被优先级高的顶替掉了
                [self dismissWithModel:lastModel isRemoveQueue:NO animation:NO];
                if (self.prohibitAutoReplaceCurrentPop == YES){
                    //不替换也要加入队列
    ...
    [self addSuperViewWithModel:model];
    //pop动画
...
+ (void)popNextViewAllGroup{
    BPopupsManager *manage = [BPopupsManager defaultManager];
    if (manage.allGroup.count > 0){
        [manage.allGroup enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            [BPopupsManager popNextViewWithGroupID:obj];
        }];
    }
}
...
+ (void)popNextViewWithGroupID:(NSString *)groupID{
    ...
    NSArray *allArr = [manage getPopupsFromAllPopupsWithGroupId:groupID];
    HLLPopupModel *lastItem = allArr.lastObject;
    //开启定时器
    if (lastItem.config.dismissDuration >= 1) {
        [lastItem startCountTime];
    }
    if (!lastItem.popupBgView.superview) {
        [manage addSuperViewWithModel:lastItem];
    }
    //pop动画
    [manage popAnimationWithPopViewModel:lastItem isNeedAnimation:YES];
}
...
- (void)dismissWithModel:(HLLPopupModel *)model isRemoveQueue:(BOOL)isRemoveQueue animation:(BOOL)animation{
    if (isRemoveQueue) {
    ...
    if (!model.config.isAloneMode && (isRemoveQueue && queueCount >= 1) && model.config.autoPop == YES) {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(model.config.dismissAnimationTime * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            if (queueCount >= 1) {
                //如果当前移除的弹窗之前还有被覆盖的，则把之前的重新展示出来
                [BPopupsManager popNextViewWithGroupID:model.config.groupID];
            }
        });
    }
    if(!animation){
...
- (void)dismissWithModel:(HLLPopupModel *)model isRemoveQueue:(BOOL)isRemoveQueue {
    [self dismissWithModel:model isRemoveQueue:isRemoveQueue animation:YES];
}
...
//隐藏弹框（没有动画）
- (void)hidenViewWithModel:(HLLPopupModel *)model{
...
- (HLLPopupModel *)getModelWithPopup:(id<BPopupInterface>)popup {
    if (!popup) {
        return nil;
    }
...
- (NSArray<id<BPopupInterface>> *)allPopups {
...
- (NSArray<NSString *> *)allGroup{
    NSMutableDictionary * groupDic = [NSMutableDictionary dictionary];
    for (HLLPopupModel *itemModel in self.windowQueue) {
        if (itemModel.config.groupID) {
            [groupDic setValue:itemModel.config.groupID forKey:itemModel.config.groupID];
        }
    }
    NSMutableArray * groupArray = [NSMutableArray arrayWithArray:groupDic.allKeys];
    return groupArray;
}
...
+ (void)pushVC:(UIViewController *)vc
{
    ...
    UIViewController * rootConroller = window.rootViewController;
    if ([rootConroller isKindOfClass:[UITabBarController class]]){
        UITabBarController * rootVC = (UITabBarController *)rootConroller;
        UIViewController * selectVC = rootVC.selectedViewController;
        if ([selectVC isKindOfClass:[UINavigationController class]]){
            UINavigationController * navVC = (UINavigationController *)selectVC;
            [navVC pushViewController:vc animated:YES];
        }else{
            [selectVC presentViewController:vc animated:YES completion:nil];
        }
    }
    else{
        
        if ([rootConroller isKindOfClass:[UINavigationController class]]){
            [(UINavigationController *)rootConroller pushViewController:vc animated:YES];
        }
        else if (rootConroller.navigationController){
            [rootConroller.navigationController pushViewController:vc animated:YES];
        }
        else{
            [rootConroller presentViewController:vc animated:YES completion:nil];
        }
    }
}
